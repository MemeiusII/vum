...
char *C_HL_extentions[] = { ".c", ".cpp", ".c", NULL };
char *C_HL_keywords[] = {
		...
		if (prev_sep) {
			...
			for (j = 0; keywords[j]; j++) {
				...
				if (!strncmp(&row->render[i], keywords[j], klen) &&
					...
					int color;
					if (kw2) color = HL_KEYWORD2;
					else if (kw3) color = HL_KEYWORD3;
					else color = HL_KEYWORD1;
					memset(&row->hl[i], color, klen);
					i += klen;
					break;
				}
				if (keywords[j] != NULL) {
...
int editorSyntaxToColor(int hl) {
	switch (hl) {
		case HL_COMMENT:
		case HL_MLCOMMENT: return 117;
		case HL_KEYWORD1: return 219;
		case HL_KEYWORD2: return 161;
		case HL_KEYWORD3: return 204;
		case HL_STRING: return 99;
		case HL_NUMBER: return 223;
		case HL_MATCH: return 80;
		default: return 231;
	}
}
...
int editorRowCxToRx(erow *row, int cx) {
	...
	for (j = 0; j < cx; j++) {
		if (row->chars[j] == '\t')
			rx += (VUM_TAB_STOP - 1) - (rx % VUM_TAB_STOP);
		rx++;
	}
	return rx;
}
...
int editorRowRxToCx(erow *row, int rx) {
	...
	for (cx = 0; cx < row->size; cx++) {
		if (row->chars[cx] == '\t')
			cur_rx += (VUM_TAB_STOP - 1) - (cur_rx % VUM_TAB_STOP);
		cur_rx++;
...
void editorUpdateRow(erow *row) {
	...
	free(row->render);
	row->render = malloc(row->size + tabs*(VUM_TAB_STOP - 1) + 1);

	int idx = 0;
	for (j = 0; j < row->size; j++) {
		if(row->chars[j] == '\t') {
			row->render[idx++] = ' ';
			while (idx % VUM_TAB_STOP != 0) row->render[idx++] = ' ';
		} else {
...
void editorProcessKeypress() {
	static int quit_times = VUM_QUIT_TIMES;
		...
		switch (c) {
			case CTRL_KEY('q'):
				if (E.dirty && quit_times > 0) {
					editorSetStatusMessage("WARNING! File have unsaved changes. Press Ctrl-Q %d more times to quit.",
							quit_times);
					quit_times--;
					return;
				}
		...
		if (filerow >= E.numrows) {
			if (E.numrows == 0 && y == E.screenrows / 3) {
				...
				int welcomelen = snprintf(welcome, sizeof(welcome),
					"(Vi) unimproved editor -- version %s", VUM_VERSION);
...
void editorDrawStatusBar(struct abuf *ab) {
	...
	abAppend(ab, "\x1b[m", 3); // <esc>[m reverts colors to normal
	abAppend(ab, "\r\n", 2);
}
...
void editorRefreshScreen() {
	...
	// Update cursor position in draw buffer
	char buf[32];
